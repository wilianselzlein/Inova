...
EOD;
$contadorrel = null;   
//debug($contadores); die;
for ($index = 0; $index < count($contadores); $index++) {

    if($contadorrel != $contadores[$index]['contador']['fantasia'])
    {
        $contadorrel = $contadores[$index]['contador']['fantasia'];
       
        $html.='  <tr>'
                .   '<td colspan="5" class="group-band">'.$contadorrel.'</td>'
                .'</tr>';
    }
    $html .= '<tr>'
            .   '<td>'.$contadores[$index]['cliente']['fantasia'].'</td>'
            .   '<td>'.$contadores[$index]['cliente']['endereco'].'</td>'
            .   '<td>'.$contadores[$index]['cliente']['bairro'].'</td>'
            .   '<td>'.$contadores[$index]['cidade']['nome'].'</td>'            
            .   '<td>'.$contadores[$index]['cliente']['telefone'].'</td>'
            . '</tr>';
    
}
$total_periodo= count($contadores);
